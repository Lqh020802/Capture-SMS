...
<view class="page">
    <view class="hero">
        <view>
            <text class="hero-title">未接来电</text>
            <text class="hero-subtitle">这里会展示最近捕获到的未接来电，方便你直接验证效果。</text>
        </view>
        <view class="hero-count">
            <text class="hero-count-num">{{ records.length }}</text>
            <text class="hero-count-label">条记录</text>
        </view>
    </view>

    <view class="toolbar">
        <text class="toolbar-tip">挂断后如果系统写入通话记录，会自动刷新到这里。</text>
    ...
    <scroll-view class="list" scroll-y>
        <view v-if="records.length === 0" class="empty-state">
            <text class="empty-text">还没有检测到未接来电</text>
        </view>
        ...
        <view v-for="(item, idx) in records" :key="idx" class="card">
            ...
            <text v-if="item.phone_number" class="phone">{{ item.phone_number }}</text>
            <text class="desc">{{ item.body || 'missed call' }}</text>
        </view>
    </scroll-view>
</view>
</template>
...
import { clearMissedCalls, loadMissedCalls } from '@/utils/missed-call-store.js'
...
export default {
    ...
    onLoad() {
        ...
        eventBus.on('missed-call', this._listener)
    },
    ...
    onUnload() {
        if (this._listener) eventBus.off('missed-call', this._listener)
    },

    methods: {
        refresh() {
            this.records = loadMissedCalls()
        },
